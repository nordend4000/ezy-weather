...
function Alerts({ alerts, timezone }) {
	return (
		<div className='border-2 border-orange-600 rounded-lg bg-white mt-8'>
			<h2 className='bg-orange-600 rounded-t-sm text-2xl pb-3 text-sky-100 text-center capitalize font-thin tracking-widest pt-3'>
			...
			{alerts.map((alert, index) => (
				<div
					key={index}
					className={`${
						index < alerts.length - 1 && "border-b-2 border-orange-600"
					} p-3 text-center`}>
					...
					<div className='p-5 text-sm text-orange-700 bg-orange-100 rounded-lg mt-3'>
						{alert.description}
					</div>
				</div>
			))}
